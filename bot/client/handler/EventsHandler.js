...
class EventsHandler {
    ...
    load = () => {
        ...
        for (const directory of readdirSync('./bot/events/')) {
            for (const file of readdirSync('./bot/events/' + directory).filter((f) => f.endsWith('.js'))) {
                try {
                    /**
                     * @type {Event['data']}
                     */
                    ...
                    error('Unable to load a event from the path: ' + 'bot/events/' + directory + '/' + file);
                }
            }
        }
