...
class CommandsHandler {
    ...
    load = () => {
        for (const directory of readdirSync('./bot/commands/')) {
            for (const file of readdirSync('./bot/commands/' + directory).filter((f) => f.endsWith('.js'))) {
                try {
                    /**
                    ...
                    error('Unable to load a command from the path: ' + 'bot/commands/' + directory + '/' + file);
                }
            }
        }
